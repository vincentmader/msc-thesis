...
\section{Preliminary Notes on Axis Discretization}
    ...
    To derive an expression that can be used for calculating the mass values at the bin centers, 
    let us first define the mass values at the \textit{bin boundaries}. To do this, consider a
    collection of $\mathcal N_m+1$ appropriately spaced grid points $m_i^\text{b}$. The first 
    and last of these values, i.e. the upper and lower boundaries of the discrete mass grid, 
    shall be given by $m_\text{max}$ and $m_\text{min}$, respectively. \\
