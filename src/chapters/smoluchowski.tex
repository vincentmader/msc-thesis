\section{The Smoluchowski Equation}
    ...
    \todo{The Smoluchowski equation...} \\
    \todo{...provides a mathematical model commonly used to describe the process of particles
    aggregation/coagulation} \\
    \todo{...is used in a wide range of physical and biological contexts. (examples?)} \\
    \todo{...is named after the Polish physicist Marian Smoluchowski, due to his treatment of the 
    equation in 1916 \cite{smoluchowski_1916}} \\

    \todo{Smoluchowski equation is a \textit{population balance equation}}. \\
    \todo{Smoluchowski equation is an \textit{integro-differential equation}}. \\
    \todo{ODE or PDE?} \\
    ...
    \subsection{Continuous Formulation}
        ...
        \todo{For the evolution of the particle mass distribution,} \\
        \todo{the Smoluchowski equation in its most general form reads:}
        ...
        Here, the term $\mathcal K(m, m', m'')$ labels the \textit{coagulation kernel}. 
        \todo{It carries the information about how the number of particles with mass $m$
        changes with time under the influence of collisions between particles with masses $m'$ 
        and $m''$.} \\
    ...
    \newpage\subsection{Discretized Formulation}
        ...
        \todo{For this, we perform the following mappings}:
        \begin{align}
            \pderiv{n}{t}
                &\to \frac{\Delta n}{\Delta t} \\
            \int_0^\infty \dd m'
                &\to \sum_{i=1}^{\mathcal N_m} \Delta m_i \\
            \int_0^\infty \dd m''
                &\to \sum_{j=1}^{\mathcal N_m} \Delta m_j
        \end{align} 
        ...
        \begin{equation}
            ...
            \begin{split}
                ...
                %         K(m,m',m'')\cdot n(m')\cdot n(m'')\ \dd m'\ \dd m''
                % \\
                %     &\downarrow
                % \\
                \pderiv{n_k}{t}
        ...
        If in addition we make use of the relationship $N_i=n_i\cdot\Delta m_i$ 
        that was defined in (\todo{cite}), then we can rewrite the Smoluchowski equation as
        \begin{equation}
            \pderiv{N_k}{t}
                = \sum_{i=1}^{\mathcal N_m}\sum_{j=1}^{\mathcal N_m} K_{kij}\cdot N_i\cdot N_j
        \end{equation}
